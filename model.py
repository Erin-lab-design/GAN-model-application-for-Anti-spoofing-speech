...
# In this code, the term "noisy" or "noise" refers to the TTS-generated speech samples,
# while "clean" or "real" refers to the genuine speech samples.
...
class Model(object):
    ...
    def save(self, save_path, step):
        ...
        self.saver.save(
            self.sess, os.path.join(save_path, model_name), global_step=step)
...
class SEGAN(Model):
    ...
    def build_model_single_gpu(self, gpu_idx):
        ...
        if gpu_idx == 0:
            ...
            #                                               self.canvas_size],
            #                                  name='sample_wavs')
            ref_Gs = self.generator(
                noisybatch, is_ref=True, spk=None, do_prelu=do_prelu)
        ...
        G, z = self.generator(
            noisybatch, is_ref=False, spk=None, do_prelu=do_prelu)
        self.Gs.append(G)
        self.zs.append(z)
        ...
        # 修改判别器的输出为分数
        d_rl_score = discriminator(self, D_rl_joint, reuse=True) 
        d_fk_score = discriminator(self, D_fk_joint, reuse=True)
        
        # 判别器标签 
        rl_audio_label = tf.ones_like(d_rl_score)
        fk_audio_label = tf.zeros_like(d_fk_score)
        # disable tensorflow summary
        '''
        ...
        # 使用sigmoid cross entropy loss
        d_rl_loss = discriminator_loss(self, rl_audio_label, d_rl_score) 
        d_fk_loss = discriminator_loss(self, fk_audio_label, d_fk_score)
        ...
        try:
            while not coord.should_stop():
                ...
                if (counter / num_devices) % config.save_freq == 0:
                    ...
                    # i do not want to log too many early, start logging wave files after 40000 steps
                    if(counter > 40000):
                        canvas_w = self.sess.run(self.Gs[0], feed_dict=fdict)
                        swaves = sample_wav
                        sample_dif = sample_wav - sample_noisy
                        for m in range(min(10, canvas_w.shape[0])):
                            print('w{} max: {} min: {}'.format(m, np.max(canvas_w[m]), np.min(canvas_w[m])))
                            wavfile.write(os.path.join(synthesis_path, 'sample_{}-{}.wav'.format(counter, m)), int(16e3),
                                de_emph(canvas_w[m], self.preemph))
                            m_gtruth_path = os.path.join(synthesis_path, 'gtruth_{}.wav'.format(m))
                            if not os.path.exists(m_gtruth_path):
                                wavfile.write(os.path.join(synthesis_path, 'gtruth_{}.wav'.format(m)), int(16e3),
                                    de_emph(swaves[m], self.preemph))
                                wavfile.write(os.path.join(synthesis_path, 'noisy_{}.wav'.format(m)), int(16e3),
                                    de_emph(sample_noisy[m], self.preemph))
                                wavfile.write(os.path.join(synthesis_path,'dif_{}.wav'.format(m)), int(16e3),
...
class SEAE(Model):
    ...
    def build_model_single_gpu(self, gpu_idx):
        ...
        if gpu_idx == 0:
            ...
            #                                               self.canvas_size],
            #                                  name='sample_wavs')
            self.reference_G = self.generator(
                noisybatch, is_ref=True, spk=None, z_on=False)
        ...
        try:
            while not coord.should_stop():
                ...
                print('{}/{} (epoch {}), g_loss = {:.5f},'
                      ' time/batch = {:.5f}, '
                      'mtime/batch = {:.5f}'.format(
                          counter, config.epoch * num_batches, curr_epoch,
                          g_loss, end - start, np.mean(batch_timings)))
                batch_idx += num_devices
                counter += num_devices
                if (counter / num_devices) % config.save_freq == 0:
                    self.save(config.save_path, counter)
                    ...
                    for m in range(min(20, canvas_w.shape[0])):
                        ...
                        wavfile.write(
                            os.path.join(save_path, 'sample_{}-{}.wav'.format(
                                counter, m)), 16e3, canvas_w[m])
                        if not os.path.exists(
                            ...
                            wavfile.write(
                                ...
                                sample_dif[m])
                        np.savetxt(
                            os.path.join(save_path, 'g_losses.txt'), g_losses)
